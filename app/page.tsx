...
const initialTransactions: Transaction[] = [
  { name: "Freelance payout", category: "Freelance", date: "Today, 9:41 AM", amount: 48200, type: "income", source: "Acme Labs", icon: "+", color: "mint" },
  ...
  { name: "Consulting retainer", category: "Consulting", date: "Jun 05", amount: 18000, type: "income", source: "Arjun Mehta", icon: "+", color: "mint" },
...
function Transactions({ transactions, onAdd }: { transactions: Transaction[]; onAdd: () => void }) { const [search, setSearch] = useState(""); const filtered = transactions.filter((t) => `${t.name} ${t.category} ${t.source || ""}`.toLowerCase().includes(search.toLowerCase())); return <><PageHeading eyebrow="LEDGER · INR" title="Transactions" description="Every rupee in and out, in one place." action={<button className="primary" onClick={onAdd}><b>＋</b> Add transaction</button>} /><div className="panel table-panel"><div className="table-tools"><div className="search-box">⌕<input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search transactions" /></div><div className="filter-pills"><button className="pill active">All</button><button className="pill">Income</button><button className="pill">Expenses</button><button className="pill">This month</button></div></div><div className="transaction-table"><div className="table-row table-header"><span>Transaction</span><span>Category</span><span>Client / source</span><span>Date</span><span>Expenses</span><span>Income</span></div>{filtered.map((t, i) => <div className="table-row" key={`${t.name}-${i}`}><span className="table-name"><i className={`transaction-icon small ${t.color}`}>{t.icon}</i><strong>{t.name}</strong></span><span>{t.category}</span><span>{t.source || "—"}</span><span>{t.date}</span><span className="table-number expense-cell">{t.type === "expense" ? money(t.amount) : "—"}</span><span className="table-number income-cell">{t.type === "income" ? money(t.amount) : "—"}</span></div>)}</div></div><div className="ledger-note"><span>i</span><div><strong>Income and expenses are kept separate</strong><p>Income rows now retain the client or source that paid you.</p></div></div></>; }


function Reports({ transactions, totals }: { transactions: Transaction[]; totals: { income: number; expense: number } }) { const expensesByCategory = transactions.filter((t) => t.type === "expense").reduce<Record<string, number>>((summary, t) => ({ ...summary, [t.category]: (summary[t.category] || 0) + t.amount }), {}); const incomeByClient = transactions.filter((t) => t.type === "income").reduce<Record<string, number>>((summary, t) => { const client = t.source || "Unassigned income"; return { ...summary, [client]: (summary[client] || 0) + t.amount }; }, {}); const expenseRows = Object.entries(expensesByCategory).sort((a, b) => b[1] - a[1]); const clientRows = Object.entries(incomeByClient).sort((a, b) => b[1] - a[1]); const savingsRate = totals.income > 0 ? Math.round(((totals.income - totals.expense) / totals.income) * 100) : 0; return <><PageHeading eyebrow="INSIGHTS · INR" title="Reports" description="See which categories cost you and which clients paid you." action={<button className="period-button">June 2025 ▾</button>} /><div className="report-kpis"><div className="panel report-kpi"><span>Total income</span><strong>{money(totals.income)}</strong><small className="green-text">Across {clientRows.length} client sources</small></div><div className="panel report-kpi"><span>Total expenses</span><strong>{money(totals.expense)}</strong><small className="coral-text">Across {expenseRows.length} categories</small></div><div className="panel report-kpi"><span>Net savings</span><strong>{money(Math.max(0, totals.income - totals.expense))}</strong><small className="green-text">{savingsRate}% of income retained</small></div></div><div className="report-grid"><div className="panel category-report"><div className="panel-head"><div><h2>Spending by category</h2><p>Where your expenses are going</p></div></div><div className="category-bars">{expenseRows.length ? expenseRows.map(([category, amount]) => <div className="category-bar" key={category}><div><span>{category}</span><strong>{money(amount)}</strong></div><div className="bar-track"><span className="expense-fill" style={{ width: `${Math.max(12, Math.min(100, (amount / Math.max(1, totals.expense)) * 100))}%` }} /></div></div>) : <p className="empty-report">Add an expense to see category insights.</p>}</div></div><div className="panel client-report"><div className="panel-head"><div><h2>Income by client</h2><p>Who has paid you so far</p></div></div><div className="client-list">{clientRows.length ? clientRows.map(([client, amount], index) => <div className="client-row" key={client}><span className="client-rank">0{index + 1}</span><span><strong>{client}</strong><small>{transactions.filter((t) => t.type === "income" && (t.source || "Unassigned income") === client).length} income entries</small></span><b>{money(amount)}</b></div>) : <p className="empty-report">Add income with a client or source to see it here.</p>}</div></div></div><div className="panel report-notes"><h2>What stands out</h2><div className="review-item"><span>01</span><div><strong>{expenseRows[0]?.[0] || "Your categories"} is your biggest spend</strong><p>{expenseRows[0] ? `${money(expenseRows[0][1])} so far this month.` : "Add expenses manually or import them from Kotak emails."}</p></div></div><div className="review-item"><span>02</span><div><strong>{clientRows[0]?.[0] || "Your clients"} is your top income source</strong><p>{clientRows[0] ? `${money(clientRows[0][1])} received so far.` : "Record who paid you when adding an income."}</p></div></div></div><div className="panel export-panel"><div><strong>Need to share this report?</strong><p>Download a clean summary of categories, clients, income, and expenses.</p></div><button className="secondary-action" onClick={() => alert(`Report ready for ${transactions.length} transactions`)}>Export report</button></div></>; }
